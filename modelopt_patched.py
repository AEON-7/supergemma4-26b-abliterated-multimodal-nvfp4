...
QUANT_ALGOS = [
    ...
    # FP4
    "NVFP4",
    # FP4 with AWQ pre-quantization scaling (ModelOpt 0.42.0+)
    "NVFP4_AWQ",
    # MXFP8
    "MXFP8",
    # MIXED_PRECISION,
...
class ModelOptNvFp4LinearMethod(LinearMethodBase):
    ...
    def create_weights(
        ...
        **extra_weight_attrs,
        ...
        # AWQ Pre-Quantization Scale (optional — present in NVFP4_AWQ models)
        # Shape: [input_size_per_partition], dtype: bfloat16
        # Initialized to ones so layers without pre_quant_scale in the
        # checkpoint are effectively no-ops.
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        ...
        # Scrub FP8 NaN values in weight_scale (ModelOpt quantizer bug)
        # FP8 E4M3 NaN is 0x7F or 0xFF - replace with 0 to zero out
        # affected weight blocks instead of propagating NaN
        ...
        if nan_count > 0:
            ...
            logger.warning(
                "Scrubbed %d FP8 NaN values in weight_scale "
                "(shape=%s, total=%d)",
                nan_count, list(layer.weight_scale.shape), ws_raw.numel(),
            )
        ...
        # Handle AWQ pre_quant_scale for runtime scaling
        if hasattr(layer, 'pre_quant_scale'):
            pqs = layer.pre_quant_scale.data.float()
            if not torch.allclose(pqs, torch.ones_like(pqs)):
                # AWQ model: pre_quant_scale was applied to activations during
                # calibration. At inference, apply the same scaling.
                layer.has_pre_quant_scale = True
                # Store as bfloat16 for efficient runtime application
                layer.pre_quant_scale_runtime = Parameter(
                    pqs.to(torch.bfloat16),
                    requires_grad=False,
                )
                logger.info(
                    "AWQ pre_quant_scale loaded: shape=%s, "
                    "min=%.4f, max=%.4f, mean=%.4f",
                    list(pqs.shape), pqs.min().item(),
                    pqs.max().item(), pqs.mean().item(),
                )
            else:
