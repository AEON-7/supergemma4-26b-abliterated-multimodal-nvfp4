    ...
    async def chat_completion_full_generator(
        ...
        for output in final_res.outputs:
            ...
            if reasoning_parser:
                ...
                # Fallback: if text-based extraction failed because
                # skip_special_tokens=True stripped reasoning delimiters,
                # re-decode token IDs with special tokens preserved and retry.
                if reasoning is None and token_ids and hasattr(
                    ...
                    if reasoning_parser.start_token_id in token_ids_list:
                        ...
                        reasoning, content = reasoning_parser.extract_reasoning(
                            full_text, request=request
                        )
                        # Clean up: re-decoding with skip_special_tokens=False
                        # may leave stray special tokens (e.g. <turn|>) in the
                        # extracted content. Re-encode then decode with stripping.
                        if content:
