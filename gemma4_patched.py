...
class Gemma4Model(nn.Module):
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        for name, loaded_weight in weights:
            ...
            # --- Per-expert NVFP4 quantized weights (compressed-tensors + modelopt) ---
            # Match names like: layers.0.moe.experts.{id}.{proj}.{suffix}
            # Map to FusedMoE fused parameters like: layers.0.moe.experts.w2_{suffix}
            # Supports both checkpoint suffix conventions:
            #   compressed-tensors: weight_packed, weight_scale, weight_global_scale, input_global_scale
            #   modelopt:           weight,        weight_scale, weight_scale_2,      input_scale, pre_quant_scale
            _nvfp4_m = re.match(
                r'(.*\.moe\.experts)\.(\d+)\.'
                r'(gate_proj|up_proj|down_proj)\.'
                r'(weight_packed|weight_scale|weight_global_scale|input_global_scale'
                r'|weight|weight_scale_2|input_scale|pre_quant_scale)$',
                name,
            )
            if _nvfp4_m:
                _prefix = _nvfp4_m.group(1)
                _expert_id = int(_nvfp4_m.group(2))
                _proj = _nvfp4_m.group(3)
                _qsuffix = _nvfp4_m.group(4)
                _fused = "w13" if _proj in ("gate_proj", "up_proj") else "w2"
                _shard = ("w1" if _proj == "gate_proj"
                          else "w3" if _proj == "up_proj" else "w2")
                # Target fused-param name in FusedMoE:
                #   {prefix}.w{N}_{suffix} (e.g. experts.w13_weight_scale)
                # ModelOptNvFp4FusedMoE registers: w{N}_weight, w{N}_weight_scale,
                # w{N}_weight_scale_2, w{N}_input_scale. Not registered: pre_quant_scale
                # (AWQ isn't wired into the FusedMoE path yet — we silently skip those
                # tensors so experts load as plain NVFP4).
                _pname = f"{_prefix}.{_fused}_{_qsuffix}"
                if _pname in params_dict:
                    _param = params_dict[_pname]
                    _wl = _param.weight_loader
                    # DEBUG: print shape/dtype mismatches at the call site
                    if (_qsuffix == "weight"
                            and loaded_weight.dtype != _param.data.dtype):
                        import sys as _sys
                        print(
                            f"[gemma4 _wl call] name={name!r} "
                            f"loaded.shape={tuple(loaded_weight.shape)} dtype={loaded_weight.dtype} "
                            f"param.shape={tuple(_param.data.shape)} dtype={_param.data.dtype} "
                            f"_pname={_pname}",
                            file=_sys.stderr, flush=True,
                        )
                    _wl(_param, loaded_weight, _qsuffix,
                        shard_id=_shard, expert_id=_expert_id)
                    loaded_params.add(_pname)
                    continue
                if _qsuffix == "pre_quant_scale":
                    # AWQ pre_quant_scale isn't supported by FusedMoE — skip silently.
                    continue
...
class Gemma4ForCausalLM(nn.Module, SupportsLoRA, SupportsPP, MixtureOfExperts):
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        def _weight_iterator():
            # DEBUG: log any bf16 expert-weight tensor surfacing from the base iterator
            import sys as _sys
            def _check(name, w):
                if (".experts." in name and name.endswith(".weight")
                        and w.dtype == torch.bfloat16):
                    print(
                        f"[iter upstream] name={name!r} shape={tuple(w.shape)} dtype={w.dtype}",
                        file=_sys.stderr, flush=True,
                    )
            ...
            for name, weight in weights:
                ...
                # Per-expert quantized weights (compressed-tensors NVFP4):
                # Checkpoint: .experts.{id}.{proj}.{quant_suffix}
                # vLLM tree:  .moe.experts.{id}.{proj}.{quant_suffix}
                # Insert .moe. before .experts.{id}. to match the
                # Gemma4DecoderLayer.moe.experts module hierarchy — but only
                # if it isn't already there (some exporters, e.g. the modelopt
                # fix_keys_for_vllm path, already add the .moe. prefix, in
                # which case this rewrite would produce .moe.moe.experts.).
                if re.search(
                    r'\.experts\.\d+\.(?:gate_proj|up_proj|down_proj)',
                    name,
                ) and ".moe.experts." not in name:
                    name = re.sub(
                        r'\.experts\.(\d+)\.',
                        r'.moe.experts.\1.',
                        name,
